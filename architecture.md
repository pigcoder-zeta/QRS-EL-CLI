...
### v2.5 新增：Agent-P 元决策层（自主模式）

v2.5 引入 **Agent-P（Planner Agent）** 作为系统的元决策层，让系统从固定流水线进化为真正的自主多 Agent 系统：
...
│                                                   │
│  自动推断语言 / 框架 / 高风险依赖                  │
│  从 vuln_catalog 中智能选择 3~10 种漏洞类型        │
│  每轮扫描后评估质量，决定 继续/重扫/终止           │
└──────────────────────┬────────────────────────────┘
                       │ 扫描计划
                       ▼
┌─── 执行层（现有 Agent）──────────────────────────┐
│  Agent-Q → CodeQL → Agent-R → Agent-S → Agent-E  │
└──────────────────────────────────────────────────┘
```

**使用方式**：`python -m src.main --source-dir ./target --auto`

Agent-P 的三大能力：

| 能力 | 说明 | LLM 调用次数 |
|---|---|---|
| 侦察（Recon） | 语言统计、框架检测、高风险依赖扫描、HTTP 入口点提取 | 0（纯规则） |
| 规划（Plan） | 基于侦察报告 + vuln_catalog，LLM 选择最优漏洞类型并排优先级 | 1 次 |
| 评估（Evaluate） | 每轮扫描后汇总结果，LLM 决策 CONTINUE/RESCAN/STOP | 每轮 1 次 |

### v2.4 全领域安全检测矩阵

| 领域 | 典型问题 | 技术方案 | 实现状态 |
|---|---|---|---|
| 注入漏洞 | SQL/命令/表达式注入、SSRF、XSS、路径穿越 | CodeQL 污点追踪 | ✅ 已完成 |
| 密码学缺陷 | 弱加密算法、硬编码凭证、不安全随机数、不安全TLS | CodeQL 模式匹配 | ✅ 已完成 |
| 内存安全(C/C++) | 缓冲区溢出、整数溢出→malloc、不安全字符串函数 | CodeQL 模式匹配 + 本地数据流 | ✅ 已完成 |
| 资源管理 | 文件/连接/Socket未关闭、内存泄露 | CodeQL 模式匹配 | ✅ 已完成 |
| 并发安全 | 竞态条件(TOCTOU)、不安全的共享状态 | CodeQL 模式匹配 | ✅ 已完成 |
| 信息泄露 | 堆栈跟踪泄露、敏感数据暴露 | CodeQL 污点追踪 | ✅ 已完成 |
| IaC 配置安全 | Terraform/K8s/Docker 错误配置 | 外部 SARIF 接入 (--external-sarif) | ✅ 已完成 |
| 移动安全(Android) | Intent 劫持、明文存储、WebView RCE、ContentProvider 泄露 | CodeQL Java + Android 专用查询 | ✅ 已完成 |
| 供应链安全(SCA) | 已知 CVE 依赖、Typosquatting 恶意包 | 包元数据解析 + OSV.dev API (--sca) | ✅ 已完成 |
| 智能合约(Solidity) | 重入攻击、未检查返回值、整数溢出、tx.origin | CodeQL 实验性 Solidity 支持 | ✅ 已完成 |
| 二进制/固件 | 危险函数导入、缓冲区溢出、固件提取分析 | Binwalk + Ghidra/Radare2 → SARIF (--firmware) | ✅ 已完成 |

### 1.1 核心设计原则

| 原则 | 说明 |
|---|---|
| **自主决策** | Agent-P 元决策层自动侦察、规划、评估，用户无需指定扫什么 |
| **神经符号融合** | LLM 负责语义理解与代码生成，CodeQL 负责精确的静态分析 |
| **多范式查询** | 污点追踪（注入类）+ 模式匹配（密码学/配置类）+ 本地数据流（内存安全类） |
